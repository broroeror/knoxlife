--- The optional Java layer: what it is, and what it has turned on.
---
--- ⚠️ THIS IS `shared`, NOT `client`, AND THAT MATTERS. It used to live in
--- client/, so on a DEDICATED SERVER none of it ran: the jar loaded and printed
--- its banner, and then nothing warmed the action groups, nothing merged a mod
--- state and nothing flipped an animset. Confirmed on pztest02 -- the log had
--- "[KnoxLife] Java layer loaded" and not one line after it.
---
--- The server needs this as much as the client does. An action group is not a
--- rendering detail: it is the animal's behaviour state machine, so the `attack`
--- state has to exist server-side for a predator to strike at all. The client
--- needs it to draw the result.
---
--- The half that talks to a PLAYER -- halo text, Say, the main-menu notice --
--- stays in client/, because a dedicated server has nobody to tell.
...
function KW.applyJavaCapabilities()
    ...
    if not loaded then
        -- ⚠️ SAY SO. This used to return silently, and silence is exactly what
        -- a dedicated server produced twice running -- the jar's own banner
        -- printed and then nothing, with no way to tell whether this file ran
        -- at all, whether the global was missing, or whether the handshake had
        -- simply decided against. A one-line report costs nothing and turns
        -- three indistinguishable failures into one answer.
        -- ...but ONLY for someone who actually has the mod. A player with no
        -- ZombieBuddy must hear nothing at all, which is the rule this file
        -- opens with, and a test asserts it. The diagnostic is for the
        -- half-installed and the server, not for the uninterested.
        if KW.javaState and KW.javaState() == "absent" then return false, {} end
        KW.log("java handshake: KnoxLifeJavaLoaded is "
               .. (KnoxLifeJavaLoaded == nil and "NOT EXPOSED to this Lua state"
                                              or "present but false")
               .. "; nothing enabled")
        return false, {}
    end
...
-- ⚠️ ACTUALLY CALL IT, AND FROM `shared`. Twice now this has been the bug:
-- first the function was defined and never invoked, then the invocation sat
-- in client/ where a dedicated server never reached it. Both times the jar
-- loaded, the log said so, and every flag stayed false.
--
-- The original note: applyJavaCapabilities() shipped defined and uninvoked,
...
-- ⚠️ TWO LATCHES, NOT ONE. A single `applied` flag conflated "the handshake
-- ran" with "the animsets were flipped", and on a dedicated server those happen
-- at different times: this file loads BEFORE the species mods call
-- registerAnimSet, so the first pass found an EMPTY plan, reported
-- "0 applied, 0 left on their fallback", set the latch, and the OnGameBoot
-- retry did nothing. The client hid it -- there the first pass failed outright,
-- so the retry was the one that did the real work.
--
-- So: the capability handshake settles once, and the flip waits until there is
-- actually a plan to apply.
local capsDone, setsDone = false, false

local function tick()
    if not capsDone then
        local live = KW.applyJavaCapabilities()
        if live then capsDone = true end
    end
    if not capsDone or setsDone then return end
    if not (KW.animsetsActive and KW.animsetsActive()) then return end
    local plan = KW.animsetPlan and KW.animsetPlan() or {}
    if #plan == 0 then return end          -- species mods have not registered yet
    pcall(KW.applyAnimSets)
    setsDone = true
end

tick()
if Events and Events.OnGameBoot then
    Events.OnGameBoot.Add(function() pcall(tick) end)
end
-- OnGameStart too: on a dedicated server the mod list is loaded in a different
-- order than on a client, and this is the last moment before animals exist.
if Events and Events.OnGameStart then
    Events.OnGameStart.Add(function() pcall(tick) end)
end
